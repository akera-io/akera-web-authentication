...
function AkeraWebAuthentication(akeraWebApp, authConfig) {
  ...
  this.init = function(config, router) {
    ...
    if (!router || typeof router.__require !== 'function')
      throw new Error('Invalid akera application router.');

    var express = router.__require('express');
    var authRouter = express.Router({
      mergeParams : true
    });

    var authRoute = config.route || '/auth/';
    authRouter.__route = router.__route + authRoute;
    authRouter.__broker = router.__broker;
    authRouter.__require = router.__require;
    ...
    router.__app.use(passport.initialize());
    router.__app.use(passport.session());
  ...
  if (akeraWebApp !== undefined) {
    ...
    router.__app = akeraWebApp.app;
    router.__require = akeraWebApp.require;
